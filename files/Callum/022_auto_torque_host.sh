...
source $1/000_config.sh
TORQUEPATH=$FILES/torque
...
$TORQUEPATH/torque-package-clients-linux-x86_64.sh --install
$TORQUEPATH/torque-package-devel-linux-x86_64.sh --install
$TORQUEPATH/torque-package-docs-linux-x86_64.sh --install
$TORQUEPATH/torque-package-server-linux-x86_64.sh --install
$TORQUEPATH/torque-package-mom-linux-x86_64.sh --install
...
cp $TORQUEPATH/init.d/pbs_server /etc/init.d
chkconfig --add pbs_server
echo /usr/local/lib > /etc/ld.so.conf.d/torque.conf
ldconfig
service pbs_server restart

cp $TORQUEPATH/init.d/pbs_sched /etc/init.d
chkconfig --add pbs_sched
echo /usr/local/lib > /etc/ld.so.conf.d/torque.conf
ldconfig
service pbs_sched restart

cp $TORQUEPATH/init.d/pbs_mom /etc/init.d
echo /usr/local/lib > /etc/ld.so.conf.d/torque.conf
